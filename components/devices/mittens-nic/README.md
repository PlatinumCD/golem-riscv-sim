...
| `0x34` | `TRACE_EVENT` | Write | Emits task start (`1`) or task finish (`2`) |
...
The four trace registers are control-plane diagnostics. A guest writes the
task ID and both halves of the 64-bit execution ID, executes an I/O fence,
and writes `TRACE_EVENT`. Under managed execution QEMU yields an fd-41 marker
carrying those IDs; SST timestamps it and resumes QEMU. No trace metadata is
injected into fd 42 or the Merlin mesh.
