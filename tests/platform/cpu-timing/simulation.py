import os

import sst

def required(name):
    value = os.environ.get(name)
    if not value:
        raise RuntimeError(f"{name} must be set")
    return value

tile = sst.Component("tile0", "mittens.tile")
tile.addParams(
    {
        "tile_id": 0,
        "qemu_path": required("MITTENS_TEST_QEMU"),
        "elf": required("MITTENS_TEST_ELF"),
        "memory": "16M",
        "launch_mode": "managed",
        "cpu_clock": "1GHz",
        "cpu_issue_width": int(required("MITTENS_CPU_ISSUE_WIDTH")),
        "sync_instruction_quantum": int(
            required("MITTENS_CPU_INSTRUCTION_QUANTUM")
        ),
        "riscv_vector_enabled": True,
        "riscv_vector_length_bits": 256,
        "riscv_vector_element_bits": 64,
        "profile_mode": "summary",
        "profile_output_directory": required("MITTENS_CPU_PROFILE"),
        "task_trace_directory": required("MITTENS_CPU_TASK_TRACE"),
        "verbose": 1,
    }
)

# Program loading and data DMA use the same shared controller.
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "support"))
from spm import attach_memory
attach_memory({0: tile})
