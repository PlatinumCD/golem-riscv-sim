...
The shared active-tile deployment launcher is
`platform/sculptor-tile-main.cpp`. This directory contains only model-suite
fixtures, configuration, and test orchestration.
...
```bash
tests/models/sculptor-ra-tree/run-test.sh gru_cell_with_bias
```
...
```bash
tests/models/sculptor-ra-tree/run-test.sh --memhierarchy gru_cell_with_bias
...
GOLEM_MODEL_CPU_ISSUE_WIDTH=2 \
  tests/models/sculptor-ra-tree/run-test.sh linear_with_bias
```
...
GOLEM_MODEL_SST_THREADS=8 \
  tests/models/sculptor-ra-tree/run-test.sh \
