...
def validate_dma(profile):
    ...
    if not completions or schedules.keys() != completions.keys():
        raise AssertionError("receive-DMA fragments did not all complete")
    expected_indices = set(range(1, len(completions) + 1))
    if completions.keys() != expected_indices:
        raise AssertionError("receive-DMA fragment indexes are not contiguous")
    for index, completion in completions.items():
        if completion["words"] != schedules[index]["words"]:
            raise AssertionError(
                f"receive-DMA fragment {index} changed its word count"
            )
    words = sum(int(row["words"]) for row in completions.values())
    if words != 300:
        raise AssertionError(
            f"receive DMA completed {words} words instead of 300"
        )
    lines = sum(
        int(row["invalidation_lines"]) for row in completions.values()
    )
    if lines <= 0:
