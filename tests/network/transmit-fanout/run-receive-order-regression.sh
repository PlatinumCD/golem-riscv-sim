...
readonly TRIAL_DIR="${OUTPUT_DIR}/receive-order"
...
readonly PAYLOAD_WORDS=1
readonly FRAME_WORDS=8
...
export MITTENS_FANOUT_ELF_PREFIX="receive-order-2"
...
set -e
if ((status != 0)); then
    echo "receive-order regression did not complete: status=${status}" >&2
    exit "${status}"
fi
...
readonly WAIT_TRACE="${PROFILE_DIR}/tile-${DESTINATION_TILE}-waits.csv"
for file in "${NETWORK_TRACE}" "${DMA_TRACE}" "${TASK_TRACE}" "${WAIT_TRACE}"; do
    if [[ ! -s "${file}" ]]; then
        echo "missing receive-order regression trace: ${file}" >&2
        exit 1
    fi
done
...
if [[ -z "${first_dma_tick}" ]]; then
    echo "receive-order regression did not schedule an RX DMA" >&2
    exit 1
fi
headers_before_dma=$(awk -F, -v tick="${first_dma_tick}" \
    ...
    "${NETWORK_TRACE}")
sources_before_dma=$(awk -F, -v tick="${first_dma_tick}" \
    'NR > 1 && $2 == "arrive" && $5 == 2 && \
     $9 == "frame-header" && $17 < tick {sources[$4]=1} \
     END {for (source in sources) count++; print count + 0}' \
    "${NETWORK_TRACE}")
if ((headers_before_dma < 5 || sources_before_dma < SOURCE_COUNT)); then
    echo "receive delay did not place five cross-source headers ahead of the first DMA" >&2
    exit 1
fi
...
zero_receive_waits=$(awk -F, \
    'NR > 1 && $3 == "nic-receive-wait" && $6 == 0 {count++} \
     END {print count + 0}' "${WAIT_TRACE}")
if ((zero_receive_waits != 0)); then
    echo "receive-order regression observed ${zero_receive_waits} zero-tick NIC receive wakeups" >&2
    exit 1
fi

echo "receive-order regression: PASS ($((SOURCE_COUNT * FANOUT)) delayed cross-source frames, header/payload FIFO preserved)"
