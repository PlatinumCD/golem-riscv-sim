...
readonly OUTPUT_DIR="${BUILD_ROOT}/tests/sculptor-eight-layer-mesh"
...
declare -a expected_cores=(0 0 1 1 3 3 2 2)
declare -a expected_local_arrays=(0 1 0 1 0 1 0 1)
declare -a expected_physical_arrays=(0 1 2 3 6 7 4 5)
for layer_id in {0..7}; do
    layer="linearwbias_${layer_id}"
    core="${expected_cores[${layer_id}]}"
    local_array="${expected_local_arrays[${layer_id}]}"
    physical_array="${expected_physical_arrays[${layer_id}]}"

    if [[ "$(grep -c \
        "sculptor.task.create.*source_layer = \"${layer}\".*sculptor.runtime.core_id = ${core} : i64" \
        "${SCHEDULED_MLIR}")" -ne 5 ]] ||
       [[ "$(grep -c \
        "sculptor.task.create.*source_layer = \"${layer}\".*sculptor.runtime.local_array_id = ${local_array} : i64.*sculptor.runtime.physical_array_id = ${physical_array} : i64" \
        "${SCHEDULED_MLIR}")" -ne 2 ]]; then
        echo "${layer} has unexpected core or analog-array placement" >&2
        exit 1
    fi
done
if ! grep -Fq \
    'sculptor.schedule.logical_array_to_analog_array = [0, 1, 2, 3, 6, 7, 4, 5]' \
    "${SCHEDULED_MLIR}" ||
   ! grep -Fq \
    'sculptor.schedule.inter_core_transfer_bytes = 48 : i64' \
    "${SCHEDULED_MLIR}"; then
    echo "the eight-layer dual-array snake schedule has unexpected metadata" >&2
    exit 1
fi
...
build_core() {
    ...
    if [[ "$(grep -c 'sculptor.task.create' \
        "${prefix}-isolated.mlir")" -ne 4 ]] ||
       [[ "$(grep 'sculptor.task.create' "${prefix}-isolated.mlir" |
        grep -c 'task_kind = \"sculptor.matrix_setup\"')" -ne 2 ]] ||
       [[ "$(grep 'sculptor.task.create' "${prefix}-isolated.mlir" |
        grep -c 'task_kind = \"mixed.fused\"')" -ne 2 ]]; then
        echo "core ${core_id} does not contain two setup and two compute tasks" >&2
        return 1
    fi
    ...
    if [[ "$("${LLVM}/bin/llvm-objdump" -d "${elf}" |
        grep -c '<unknown>')" -lt 8 ]]; then
        echo "core ${core_id} lacks its eight analog operations" >&2
        return 1
    fi
    for symbol in \
        golem_tile_core_id \
...
export MITTENS_SCULPTOR_EIGHT_LAYER_STATS="${STATISTICS}"
for core_id in 0 1 2 3; do
    export "MITTENS_SCULPTOR_EIGHT_LAYER_CORE${core_id}_ELF=${OUTPUT_DIR}/core-${core_id}.elf"
done
...
grep -Fq \
    "[core 0] all eight matrices ready; dispatching model input" \
    "${SIMULATION_OUTPUT}"
grep -Fq \
    "[core 2] final model output [56,42,42,60]" \
    "${SIMULATION_OUTPUT}"
grep -Fq \
    "SCULPTOR_EIGHT_LAYER_2X2_DUAL_ARRAY_PASS" \
    "${SIMULATION_OUTPUT}"

if [[ ! -s "${STATISTICS}" ]]; then
    echo "the eight-layer proof did not produce router statistics" >&2
    exit 1
fi
if ! awk -F, '
    $1 ~ /^router_/ &&
    ...
    END {
        valid = ejected_bits == 512 &&
                physical_link_bits == 544 &&
                !invalid_width
        exit !valid
    }
' "${STATISTICS}"; then
    echo "unexpected 2x2 route traffic or non-32-bit transfer" >&2
    exit 1
fi
...
completion_time="$(awk \
    ...
    "${SIMULATION_OUTPUT}")"
echo "eight-layer PyTorch -> four dual-array ELFs -> 2x2 mesh: PASS"
