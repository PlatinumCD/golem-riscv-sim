...
readonly OUTPUT_DIR="${BUILD_ROOT}/tests/sculptor-four-layer-mesh"
...
readonly LINALG_MLIR="${OUTPUT_DIR}/four-linear-linalg.mlir"
readonly ISLANDS_MLIR="${OUTPUT_DIR}/four-linear-islands.mlir"
readonly SCHEDULED_MLIR="${OUTPUT_DIR}/four-linear-scheduled.mlir"
readonly FUSED_MLIR="${OUTPUT_DIR}/four-linear-fused.mlir"
...
if [[ "$(grep -c 'linalg.matmul' "${LINALG_MLIR}")" -ne 4 ]]; then
    echo "Torch-MLIR did not emit four linear matmuls" >&2
    exit 1
fi
...
"${SCULPTOR_OPT}" "${ISLANDS_MLIR}" \
    --sculptor-schedule-task-graph="cores=4 arrays-per-core=1 topology=mesh mesh-rows=2 mesh-cols=2 schedule=snake" \
    -o "${SCHEDULED_MLIR}"
...
for layer in "${!expected_cores[@]}"; do
    core="${expected_cores[${layer}]}"
    if [[ "$(grep -c \
        "sculptor.task.create.*source_layer = \"${layer}\".*sculptor.runtime.core_id = ${core} : i64" \
        "${SCHEDULED_MLIR}")" -ne 5 ]]; then
        echo "${layer} was not placed entirely on core ${core}" >&2
        exit 1
    fi
done
if ! grep -Fq \
    'sculptor.schedule.logical_array_to_analog_array = [0, 1, 3, 2]' \
    "${SCHEDULED_MLIR}" ||
   ! grep -Fq \
    'sculptor.schedule.inter_core_transfer_bytes = 48 : i64' \
    "${SCHEDULED_MLIR}"; then
    echo "the four-layer snake schedule has unexpected placement metadata" >&2
    exit 1
fi
...
if [[ "$(grep -c 'sculptor.task.create' "${FUSED_MLIR}")" -ne 8 ]] ||
   [[ "$(grep 'sculptor.task.create' "${FUSED_MLIR}" |
        grep -c 'task_kind = \"sculptor.matrix_setup\"')" -ne 4 ]] ||
   [[ "$(grep 'sculptor.task.create' "${FUSED_MLIR}" |
        grep -c 'task_kind = \"mixed.fused\"')" -ne 4 ]]; then
    echo "Sculptor did not fuse the graph to four setup and four compute tasks" >&2
    exit 1
fi
...
for shim in set load compute store; do
    if [[ "$(grep -c "call @golem_analog_mvm_${shim}" \
        "${LLVM_SHIMS_MLIR}")" -ne 4 ]]; then
        echo "expected four ${shim} shim calls" >&2
        exit 1
    fi
done
...
grep -Fq \
    "[core 0] all four matrices ready; dispatching model input" \
    "${SIMULATION_OUTPUT}"
grep -Fq \
    "[core 2] final model output [21,10,20,15]" \
    "${SIMULATION_OUTPUT}"
grep -Fq "SCULPTOR_FOUR_LAYER_2X2_PASS" "${SIMULATION_OUTPUT}"
