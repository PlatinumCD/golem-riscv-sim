    ...
    const std::vector<float>& matrix)
{
    ...
    target.activeRows = 0;
    target.activeColumns = 0;
    for (std::uint32_t row = 0; row < arrayRows_; ++row) {
        for (std::uint32_t column = 0; column < arrayColumns_; ++column) {
            const std::size_t index =
                static_cast<std::size_t>(row) * arrayColumns_ + column;
            if (target.matrix[index] == 0.0F)
                continue;
            target.activeRows = std::max(target.activeRows, row + 1);
            target.activeColumns =
                std::max(target.activeColumns, column + 1);
        }
    }
    ...
    const std::vector<float>& input)
{
    ...
    target.input = input;
    target.inputFinite = std::all_of(
        target.input.begin(), target.input.end(),
...
void NativeAnalogBackend::compute(std::uint32_t arrayId)
{
    ...
    // Sculptor zero-pads logical matrices to the physical array shape. The
    // hardware latency is modeled independently by AnalogDevice, so evaluating
    // those known-zero rows and columns only adds host work. Non-finite input
    // falls back to the full IEEE-754 computation because zero multiplied by
    // infinity or NaN is observable.
    const std::uint32_t rows =
        target.inputFinite ? target.activeRows : arrayRows_;
    const std::uint32_t columns =
        target.inputFinite ? target.activeColumns : arrayColumns_;
    target.lastComputeMultiplyCount =
...
std::uint64_t NativeAnalogBackend::lastComputeMultiplyCount(
    std::uint32_t arrayId) const
{
    return array(arrayId).lastComputeMultiplyCount;
}
    ...
    std::uint32_t destinationArrayId)
{
    ...
    destination.inputFinite = std::all_of(
        destination.input.begin(), destination.input.end(),
        [](float value) { return std::isfinite(value); });
