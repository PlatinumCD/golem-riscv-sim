...
class PacketEvent final : public SST::Event
{
  public:
    ...
    struct Metadata {
        ...
        // Optional local-injection lane selected by the tile's TX frontend.
        // This is endpoint-local metadata; routers still route every lane to
        // any cardinal output using the ordinary routing/arbitration rules.
        std::uint32_t injectionLane = UINT32_MAX;
    };
    ...
    void serialize_order(SST::Core::Serialization::serializer& ser) override
    {
        ...
        SST_SER(metadata_.logicalIteration);
        SST_SER(metadata_.protocolWords);
        SST_SER(metadata_.payloadWords);
        SST_SER(metadata_.injectionLane);
    }
