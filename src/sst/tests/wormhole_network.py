...
LINK_LATENCY = "1ns"
BUFFER_FLITS = int(
    os.environ.get("MITTENS_WORMHOLE_BUFFER_FLITS", "32")
)
PAYLOAD_WORDS = int(
    os.environ.get("MITTENS_WORMHOLE_PAYLOAD_WORDS", "16")
)
RECEIVE_CYCLE = int(
    os.environ.get("MITTENS_WORMHOLE_RECEIVE_CYCLE", "0")
)
SOURCE_ROUTER_CREDITS = int(
    os.environ.get(
        "MITTENS_WORMHOLE_SOURCE_ROUTER_CREDITS",
        str(BUFFER_FLITS),
    )
)
LINK_WIDTH_BITS = int(
...
for y in range(HEIGHT):
    for x in range(WIDTH):
        ...
        if node == 3:
            ...
            probe_params["receive_cycle"] = RECEIVE_CYCLE
