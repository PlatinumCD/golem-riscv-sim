...
AnalogBridgeSubmission
SharedAnalogMemoryBridge::takeAndAcceptSubmission(
    const AnalogBridgeToken& token)
{
    std::optional<AnalogBridgeSubmission> submission =
        nextSubmission(token.arrayId);
    if (!submission.has_value() ||
        submission->token.arrayId != token.arrayId ||
        submission->token.sequence != token.sequence) {
        setProtocolError(MITTENS_ANALOG_BRIDGE_ERROR_BAD_SLOT);
        throw std::runtime_error(
            "analog bridge deferred acceptance token was not next");
    }

    // Copy the complete command before publishing ACCEPTED. The guest is
    // allowed to publish its next command as soon as acceptance is visible,
    // while the returned immutable value can be held until its modeled SST
    // submission frontier without consulting the shared slot again.
    AnalogBridgeSubmission accepted = std::move(*submission);
    markAccepted(token);
    return accepted;
}
