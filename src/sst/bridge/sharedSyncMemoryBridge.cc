    ...
    std::uint64_t instructionBudget)
{
    ...
    mapping_->global_dma_logical_iteration = 0;
    mapping_->global_dma_scratchpad_offset = 0;
    mapping_->global_dma_request_flags = 0;
    mapping_->epoch_id = UINT32_MAX;
    mapping_->epoch_contribution = UINT32_MAX;
    mapping_->memory_batch_count = 0;
    mapping_->global_dma_batch_count = 0;
    mapping_->analog_batch_count = 0;
    ...
    std::chrono::microseconds spinTimeout)
{
    ...
    std::uint32_t state = mittens_sync_load_acquire(&mapping_->state);
    if (spinTimeout > std::chrono::microseconds::zero() &&
        (state != MITTENS_SYNC_STATE_EVENT ||
         mapping_->event_sequence == observedEventSequence_)) {
        const auto deadline =
            std::chrono::steady_clock::now() + spinTimeout;
        do {
            state = mittens_sync_load_acquire(&mapping_->state);
    ...
    if (state == MITTENS_SYNC_STATE_EVENT &&
        ...
        QemuSyncEvent event{
            ...
            mapping_->rx_dma_logical_iteration,
            mapping_->rx_dma_word_count,
            mapping_->memory_address,
            mapping_->memory_size,
            mapping_->memory_flags,
            mapping_->global_dma_logical_iteration,
            mapping_->global_dma_scratchpad_offset,
            mapping_->global_dma_request_flags,
            mapping_->epoch_id,
            mapping_->epoch_contribution,
            {},
            {},
            {},
        };
        ...
        if (carriesMemoryBatch) {
            if (mapping_->memory_batch_count == 0 ||
                mapping_->memory_batch_count >
            ...
            event.memoryBatch.assign(
                records, records + mapping_->memory_batch_count);
        } else if (mapping_->memory_batch_count != 0) {
            setProtocolError(MITTENS_SYNC_BRIDGE_ERROR_BAD_BUDGET);
            throw std::runtime_error(
                "QEMU supplied an unmarked memory batch");
        }
        const bool carriesGlobalDMABatch =
            event.stopReason ==
        ...
        if (carriesGlobalDMABatch) {
            if (mapping_->global_dma_batch_count == 0 ||
                mapping_->global_dma_batch_count >
                    MITTENS_SYNC_GLOBAL_DMA_BATCH_CAPACITY ||
                !event.memoryBatch.empty()) {
                setProtocolError(MITTENS_SYNC_BRIDGE_ERROR_BAD_BUDGET);
                throw std::runtime_error(
                    "QEMU supplied an invalid global DMA batch");
            }
            const auto* records =
                mittens_sync_global_dma_batch_const(mapping_);
            event.globalDMASubmitBatch.assign(
                records, records + mapping_->global_dma_batch_count);
        } else if (mapping_->global_dma_batch_count != 0) {
            setProtocolError(MITTENS_SYNC_BRIDGE_ERROR_BAD_BUDGET);
            throw std::runtime_error(
                "QEMU supplied an unmarked global DMA batch");
        }
        const bool carriesAnalogBatch =
            event.stopReason == MITTENS_SYNC_STOP_ANALOG_SUBMIT_BATCH ||
            (event.flags & MITTENS_SYNC_EVENT_FLAG_ANALOG_BATCH) != 0;
        if (carriesAnalogBatch) {
            if (mapping_->analog_batch_count == 0 ||
                mapping_->analog_batch_count >
                    MITTENS_SYNC_ANALOG_BATCH_CAPACITY ||
                !event.memoryBatch.empty() ||
                !event.globalDMASubmitBatch.empty()) {
                setProtocolError(MITTENS_SYNC_BRIDGE_ERROR_BAD_BUDGET);
                throw std::runtime_error(
                    "QEMU supplied an invalid analog submit batch");
            }
            const auto* records =
                mittens_sync_analog_batch_const(mapping_);
            event.analogSubmitBatch.assign(
                records, records + mapping_->analog_batch_count);
        } else if (mapping_->analog_batch_count != 0) {
            setProtocolError(MITTENS_SYNC_BRIDGE_ERROR_BAD_BUDGET);
            throw std::runtime_error(
                "QEMU supplied an unmarked analog submit batch");
        }
        return event;
    }
