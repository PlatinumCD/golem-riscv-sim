...
fixed 192-byte control contract. SST publishes a grant epoch and instruction
...
ABI version 18 adds `EPOCH_BARRIER_ARRIVE`, an exact completed-epoch ID, and
an explicit work-complete or idle contribution. The event blocks its QEMU
process until the SST deployment controller returns the matching release.
ABI version 19 permits an ordered memory-access batch to share the same fd 41
publication as the semantic stop event that follows it. The
`MEMORY_BATCH` event flag removes a second host handshake; SST still replays
the batch before the semantic event and therefore preserves architectural
order.
ABI version 20 adds exact contiguous-run encoding for adjacent scratchpad
accesses from one dynamic instruction. A 64-byte memory record carries the
first address, access size, and `repeat_count`. The active batch limit counts
logical accesses rather than compact transport records, so enabling run
compaction preserves the same flush and arbitration boundaries as the
uncompacted execution.
ABI version 26 replaces the serial-order assumption in compiler-certified
global-DMA macros with an explicit ordered event tape. Each 88-byte physical
request record carries submit and wait instruction boundaries plus two event
ordinals. SST validates that those ordinals form one complete token-lifecycle
permutation, then replays interleaved submits and waits without changing any
physical request or modeled controller decision.
The current synchronization ABI is version 26. Its control structure remains
exactly 192 bytes, each memory record remains exactly 64 bytes, and the mapping
contains space for 1,024 memory records, eight global-DMA records, and 1,024
analog-submit records.
...
| `0x24` | `rx_dma_completion_write_index` | RX-DMA completions published by QEMU |
| `0x28` | `rx_dma_completion_read_index` | RX-DMA completions acknowledged by the guest |
