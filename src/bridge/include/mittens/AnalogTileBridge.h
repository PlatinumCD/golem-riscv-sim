...
enum MittensAnalogCommandFlags {
    /*
     * SetMatrix carries only the valid row-major rectangle. The device
     * treats every physical row/column outside that rectangle as zero.
     */
    MITTENS_ANALOG_COMMAND_FLAG_COMPACT_SET_MATRIX = UINT32_C(1) << 0,
};

#define MITTENS_ANALOG_SET_MATRIX_ARRAY_MASK UINT32_C(0xff)
#define MITTENS_ANALOG_SET_MATRIX_ROWS_SHIFT UINT32_C(8)
#define MITTENS_ANALOG_SET_MATRIX_SHAPE_MASK UINT32_C(0xfff)
#define MITTENS_ANALOG_SET_MATRIX_COLUMNS_SHIFT UINT32_C(20)
...
static inline uint32_t mittens_analog_set_matrix_array_id(
    const MittensAnalogCommand *command)
{
    return (uint32_t)command->operand1 &
           MITTENS_ANALOG_SET_MATRIX_ARRAY_MASK;
}

static inline uint32_t mittens_analog_set_matrix_valid_rows(
    const MittensAnalogCommand *command)
{
    return ((uint32_t)command->operand1 >>
...
static inline uint32_t mittens_analog_set_matrix_valid_columns(
    const MittensAnalogCommand *command)
{
    return ((uint32_t)command->operand1 >>
