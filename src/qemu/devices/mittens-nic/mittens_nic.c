...
#define MITTENS_NIC_RX_DMA_LOGICAL_ITERATION_LOW 0x64
#define MITTENS_NIC_RX_DMA_LOGICAL_ITERATION_HIGH 0x68
#define MITTENS_NIC_RX_DMA_COMPLETION_ITERATION_LOW 0x6c
#define MITTENS_NIC_RX_DMA_COMPLETION_ITERATION_HIGH 0x70
#define MITTENS_NIC_RX_SOFTWARE_CLAIM 0x74
...
#define MITTENS_NIC_RX_DMA_CAPACITY MITTENS_BRIDGE_QUEUE_CAPACITY
...
typedef struct MittensNICState {
    ...
    uint64_t receive_dma_logical_iteration;
    ...
    uint64_t receive_dma_submission_sequence;
} MittensNICState;
    ...
    MittensNICReceiveDMA *descriptor)
{
    ...
    completion->logical_iteration = descriptor->logical_iteration;
    ++s->receive_dma_completion_write;
    if (!mittens_bridge_rx_dma_publish_completion(s->bridge)) {
        --s->receive_dma_completion_write;
        mittens_nic_set_error(
            s,
            MITTENS_BRIDGE_ERROR_DMA,
            "shared RX DMA completion queue is full");
        return;
    }
    ...
    MittensNICState *s)
{
    ...
    software_visible =
        mittens_bridge_rx_burst_software_visible(s->bridge);
    /*
    ...
    descriptor = mittens_nic_find_receive_dma(s, packet.source);
    if (!software_visible) {
        return false;
    }
                                 ...
                                 unsigned int size)
{
    ...
    case MITTENS_NIC_RX_DMA_COMPLETION_ITERATION_LOW:
        if (!mittens_nic_receive_dma_completion_valid(s)) {
            mittens_nic_set_error(
                s, MITTENS_BRIDGE_ERROR_BAD_MMIO,
                "RX DMA completion iteration read while no completion exists");
            return 0;
        }
        return (uint32_t)s->receive_dma_completions[
            s->receive_dma_completion_read %
            MITTENS_NIC_RX_DMA_CAPACITY].logical_iteration;

    case MITTENS_NIC_RX_DMA_COMPLETION_ITERATION_HIGH:
        if (!mittens_nic_receive_dma_completion_valid(s)) {
            mittens_nic_set_error(
                s, MITTENS_BRIDGE_ERROR_BAD_MMIO,
                "RX DMA completion iteration read while no completion exists");
        ...
        return (uint32_t)(s->receive_dma_completions[
            s->receive_dma_completion_read %
            MITTENS_NIC_RX_DMA_CAPACITY].logical_iteration >> 32);
    ...
    case MITTENS_NIC_RX_DMA_LOGICAL_ITERATION_LOW:
    case MITTENS_NIC_RX_DMA_LOGICAL_ITERATION_HIGH:
    case MITTENS_NIC_RX_SOFTWARE_CLAIM:
        mittens_nic_set_error(s, MITTENS_BRIDGE_ERROR_BAD_MMIO,
                              ...
                              uint64_t value, unsigned int size)
{
    ...
    case MITTENS_NIC_TRACE_EVENT:
        ...
        if (value == MITTENS_NIC_TRACE_EVENT_EPOCH_BARRIER_ARRIVE) {
            if (s->trace_execution_id > MITTENS_SYNC_EPOCH_IDLE) {
                mittens_nic_set_error(
                    s,
                    MITTENS_BRIDGE_ERROR_BAD_MMIO,
                    "epoch barrier arrival used an invalid contribution");
                return;
            }
            mittens_sync_yield_epoch(
                s->trace_task_id,
                (uint32_t)s->trace_execution_id);
            return;
        }
        mittens_nic_set_error(
            s,
    ...
    case MITTENS_NIC_RX_DMA_LOGICAL_ITERATION_LOW:
        s->receive_dma_logical_iteration =
            (s->receive_dma_logical_iteration &
    ...
    case MITTENS_NIC_RX_DMA_SUBMIT: {
        ...
        if (s->bridge == NULL ||
            ...
            (s->receive_dma_logical_iteration != UINT64_MAX &&
             !mittens_sync_scratchpad_contains(
                 s->receive_dma_address, byte_count)) ||
        ...
        descriptor->logical_iteration =
            s->receive_dma_logical_iteration;
        descriptor->submission_sequence =
            s->receive_dma_submission_sequence++;
    ...
    case MITTENS_NIC_RX_SOFTWARE_CLAIM:
        if (s->bridge == NULL ||
            s->receive_dma_route_id == UINT32_MAX ||
            s->receive_dma_word_count == 0 ||
            !mittens_sync_available()) {
            mittens_nic_set_error(
                s,
                MITTENS_BRIDGE_ERROR_DMA,
                "invalid software receive-payload claim");
            return;
        }
        mittens_sync_yield_receive_software_claim(
            s->receive_dma_source,
            s->receive_dma_route_id,
            s->receive_dma_logical_iteration,
            s->receive_dma_word_count);
        return;
    ...
    case MITTENS_NIC_RX_DMA_COMPLETION_ACK:
        ...
        ++s->receive_dma_completion_read;
        if (!mittens_bridge_rx_dma_consume_completion(s->bridge)) {
            --s->receive_dma_completion_read;
            mittens_nic_set_error(
                s,
                MITTENS_BRIDGE_ERROR_DMA,
                "shared RX DMA completion queue is empty");
        }
        return;
